...
export default function Body() {
  const [listofRes, setList] = useState(Data);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const resData = await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=12.283007&lng=76.6476158&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING");
  ...
  {Array.isArray(listofRes) && listofRes.map((restaurant) => (
